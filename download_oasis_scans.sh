...
# <scan_type> - The scan type of the scan you want to download. (e.g. T1w, angio, bold, fieldmap, FLAIR)
...
if [ ${#@} == 0 ]; then
    ...
    echo "<scan_type>: scan type you would like to download (e.g. T1w). You can also enter multiple comma-separated scan types (e.g. swi,T2w)"   
else 
    ...
    sed 1d $INFILE | while IFS=, read -r EXPERIMENT_ID; do
        ...
        if zip -Tq $DIRNAME/$EXPERIMENT_ID.zip > /dev/null; then
            ...
            echo "Unzipping scan and rearranging files."
